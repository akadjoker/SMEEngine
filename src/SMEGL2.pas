...
interface
uses
Classes,  math, SysUtils,sdl2,sdl_simple,sdl2_image, glmath, GLES2;
...
ucolor_frag_shader='#ifdef GL_ES'+#10+
'precision lowp float;					'+#10+
'#endif									'+#10+
'varying vec4 v_fragmentColor;			'+#10+
'void main()								'+#10+
'{									'+#10+
'	gl_FragColor = v_fragmentColor;		'+#10+
'}';

ucolor_pixel_shader='attribute vec2 a_position;		'+#10+
'uniform	mat4 u_projection;							'+#10+
'uniform	vec4 u_color;								'+#10+
'uniform float u_pointSize;							'+#10+
'#ifdef GL_ES										'+#10+
'varying lowp vec4 v_fragmentColor;		'+#10+
'#else												'+#10+
'varying vec4 v_fragmentColor;			'+#10+
'#endif												'+#10+
'void main()											'+#10+
...
const
  ...
  type
    ...
    PColorvertex=^Colorvertex;
    ColorvertexArray=array [0..MaxInt div 32 - 1] of Colorvertex;
    PColorvertexArray=^ColorvertexArray;
...
UNIFORM_TEXTURE,
UNIFORM_PROJECTION:TGLint;
public


procedure  Load(vertexs,fragments:string);
procedure  setString(vertexs,fragments:string);


procedure setMatrix(mat:Matrix);overload;
procedure setMatrix( value:pGLfloat);overload;
procedure setTexture(layer:integer=0);
...
TSMEUColorShader=class
private
VertexShader,FragmentShader,ShaderProgram:TGLuint;
UNIFORM_PROJECTION:TGLint;
UNIFORM_COLOR:TGLint;
public
...
TSMESpriteCloud = Class
  ...
  public
    ...
    countvertex:integer;
      ...
      constructor Create(maxSprites:integer;RenderTexture:PSDL_Texture;shader:TSMEShader);
...
//
 ...
 TSMEPrmitives=class
    private
  bVertices   : array of Colorvertex;
  bSize       : Integer;
      Fcolor:HColor;
      shader:TSMEColorShader;
   public
   constructor Create;
   destructor Destroy;
   ...
   procedure Ubind;
   procedure setColor(color:Uint32) ;
   procedure PutPixel(x,y:single; Color: Cardinal);

        procedure glVertex2f(x, y: GLfloat);
        procedure glColor3f(r,g,b: GLfloat);


   procedure Line(x1, y1, x2, y2 : single; color : LongWord); overload;
   procedure Line(x1, y1, x2, y2 : single; r,g,b:single);overload;

   procedure HLine(x1, x2, y : single; color : LongWord);
   procedure VLine(y1, y2, x : single; color : LongWord);

   procedure FillCircle(x, y, r : single; color : LongWord);
  procedure FillEllipse(x, y, rx, ry : single; color : LongWord);
  procedure CubicBezierLine(x1, y1, x2, y2, c1x, c1y, c2x, c2y:single; segments : integer; color : LongWord);
  procedure Rect(x1, y1, x2, y2 : single; color : LongWord);
  procedure Ellipse(x, y, rx, ry : single; color : LongWord);
  procedure Circle(x, y:single; r : single; color : LongWord);
   ...
   procedure Triangles(Points: array of Colorvertex; NumPoints: Integer);
   procedure Lines(Points: array of Colorvertex; NumPoints: Integer);
...
screen_width:integer  = 840;
screen_height:integer = 640;

// This is your target virtual resolution for the game, the size you built your game to
virtual_width:integer=480;
virtual_height:integer=320;
...
ATTRIBUTE_POSITION:TGLint=0;
ATTRIBUTE_TEXCOORD:TGLint=1;
ATTRIBUTE_COLOR:TGLint=2;
...
constructor TSMESpriteCloud.Create(maxSprites:integer;RenderTexture:PSDL_Texture;shader:TSMEShader);
var
  i,  N,X:INTEGER;
begin
...
// glBindAttribLocation(shader.ShaderProgram,GLES2_ATTRIBUTE_POSITION,'a_position');
// glBindAttribLocation(shader.ShaderProgram,GLES2_ATTRIBUTE_TEXCOORD,'a_texCoord');
// glBindAttribLocation(shader.ShaderProgram,GLES2_ATTRIBUTE_COLOR,'a_color');
...
begin
  ...
  FCurBlendMode:=-1;
//  writeln(sizeof (hgeVertex));

  glVertexAttribPointer(ATTRIBUTE_POSITION, 2, GL_FLOAT, GL_FALSE,  sizeof (hgeVertex), @FVertArray[0].x);
 // glEnableVertexAttribArray(ATTRIBUTE_POSITION);

  glVertexAttribPointer(ATTRIBUTE_COLOR, 4, GL_FLOAT, GL_FALSE,  sizeof (hgeVertex), @FVertArray[0].col);
 // glEnableVertexAttribArray(ATTRIBUTE_COLOR);


  glVertexAttribPointer(ATTRIBUTE_TEXCOORD, 2, GL_FLOAT, GL_FALSE,  sizeof (hgeVertex), @FVertArray[0].tx);
...
begin
 ...
 if Success=0 then //<>GL_TRUE then
 begin
  ...
  begin
   ...
   sdl_log(pchar('shader result:'+ShaderType+': '+Buffer));
   freemem(Buffer);
  end;
 end
end;
...
procedure  TSMEShader.setString(vertexs,fragments:string);
begin

 ShaderProgram:=glCreateProgram();
 VertexShader:=CreateShader(GL_VERTEX_SHADER,PChar(vertexs));

 FragmentShader:=CreateShader(GL_FRAGMENT_SHADER,PChar(fragments));
 glAttachShader(ShaderProgram,VertexShader);
 glAttachShader(ShaderProgram,FragmentShader);

 glBindAttribLocation(ShaderProgram,ATTRIBUTE_POSITION,'a_position');
 glBindAttribLocation(ShaderProgram,ATTRIBUTE_TEXCOORD,'a_texCoord');
 glBindAttribLocation(ShaderProgram,ATTRIBUTE_COLOR,'a_color');
 glLinkProgram(ShaderProgram);
 UNIFORM_PROJECTION:=glGetUniformLocation(ShaderProgram,'u_projection');
 UNIFORM_TEXTURE:=glGetUniformLocation(ShaderProgram,'u_texture');
 glUseProgram(ShaderProgram);
end;


procedure  TSMEShader.Load(vertexs,fragments:string);
var
  Data: Pointer;
  Size: integer;
...
begin
 ...
 ShaderProgram:=glCreateProgram();
  ...
  if (Data = nil) then
  begin
          SDL_Log(pchar('fail load vertx shader'+ vertexs));
  Exit;
  end;
  ...
  if (Data = nil) then
  begin
  SDL_Log(pchar('fail load fragment shader'+ fragments));
  Exit;
  end;
...
procedure TSMEShader.setMatrix(mat:Matrix);
begin
glUniformMatrix4fv(UNIFORM_PROJECTION, 1, GL_FALSE, @mat.v[0]);
end;
procedure TSMEShader.setMatrix( value:pGLfloat);
begin
glUniformMatrix4fv(UNIFORM_PROJECTION, 1, GL_FALSE,value);
end;
procedure TSMEShader.setTexture(layer:integer=0);
begin
glActiveTexture(GL_TEXTURE0);
glUniform1i(UNIFORM_TEXTURE, layer);
end;



procedure TSMEShader.Bind();
begin
 glUseProgram(ShaderProgram);
 glEnableVertexAttribArray(ATTRIBUTE_POSITION);
 glEnableVertexAttribArray(ATTRIBUTE_COLOR);
...
procedure TSMEShader.unBind();
begin
     glUseProgram(0);
end;
//*******************************************

procedure  TSMEUColorShader.Init;
begin

 ShaderProgram:=glCreateProgram();
 VertexShader:=CreateShader(GL_VERTEX_SHADER,PChar(ucolor_pixel_shader));
 FragmentShader:=CreateShader(GL_FRAGMENT_SHADER,PChar(ucolor_frag_shader));
 glAttachShader(ShaderProgram,VertexShader);
 glAttachShader(ShaderProgram,FragmentShader);
 ...
 glLinkProgram(ShaderProgram);
 UNIFORM_PROJECTION:=glGetUniformLocation(ShaderProgram,'u_projection');
 UNIFORM_COLOR:=glGetUniformLocation(ShaderProgram,'u_color');
 glUseProgram(ShaderProgram);
end;


destructor TSMEUColorShader.Destroy;
begin
 glDeleteProgram(ShaderProgram);
 glDeleteShader(FragmentShader);
 glDeleteShader(VertexShader);
...
procedure TSMEUColorShader.setColor(r,g,b:single;a:single=1);
begin
glUniform4f(UNIFORM_COLOR,r,g,b,a);

end;

procedure TSMEUColorShader.setMatrix(mat:Matrix);
begin
glUniformMatrix4fv(UNIFORM_PROJECTION, 1, GL_FALSE, @mat.v[0]);
end;
procedure TSMEUColorShader.setMatrix( value:pGLfloat);
begin
glUniformMatrix4fv(UNIFORM_PROJECTION, 1, GL_FALSE,value);
end;
...
procedure TSMEUColorShader.Bind();
begin
 glUseProgram(ShaderProgram);
 glEnableVertexAttribArray(ATTRIBUTE_POSITION);
 glDisableVertexAttribArray(ATTRIBUTE_COLOR);
...
procedure TSMEUColorShader.unBind();
begin
     glUseProgram(0);
end;
...
procedure  TSMEColorShader.setString(vertexs,fragments:string);
begin
...
procedure  TSMEColorShader.Load(vertexs,fragments:string);
var
  Data: Pointer;
  Size: integer;
...
destructor TSMEColorShader.Destroy;
begin
 glDeleteProgram(ShaderProgram);
 glDeleteShader(FragmentShader);
 glDeleteShader(VertexShader);
...
procedure TSMEColorShader.setMatrix(mat:Matrix);
begin
glUniformMatrix4fv(UNIFORM_PROJECTION, 1, GL_FALSE, @mat.v[0]);
end;
procedure TSMEColorShader.setMatrix( value:pGLfloat);
begin
glUniformMatrix4fv(UNIFORM_PROJECTION, 1, GL_FALSE,value);
end;
...
procedure TSMEColorShader.Bind();
begin
 glUseProgram(ShaderProgram);
 glEnableVertexAttribArray(ATTRIBUTE_POSITION);
 glEnableVertexAttribArray(ATTRIBUTE_COLOR);
...
procedure TSMEColorShader.unBind();
begin
     glUseProgram(0);
end;
...
matScale:=MatrixScale(scale_x,scale_y,1);
...
procedure TSMECameraOrtho.resizeGame;
var
targetAspectRatio:single;
vp_x,vp_y,width,height:integer;
begin

targetAspectRatio := virtual_width/virtual_height;

// figure out the largest area that fits in this resolution at the desired aspect ratio
 width := screen_width ;
 height := round((width / targetAspectRatio + 0.5));

if (height > screen_height ) then
begin
   //It doesn't fit our height, we must switch to pillarbox then
    height := screen_height ;
    width := round(height * targetAspectRatio + 0.5);
end;

// set up the new viewport centered in the backbuffer
 vp_x := (screen_width  div 2) - (width div 2);
 vp_y := (screen_height div 2) - (height div 2);
...
procedure TSMEPrmitives.PutPixel(x,y:single; Color: Cardinal);
begin


Line(x,y,x+1,y+1,color);
end;


procedure TSMEPrmitives.Line(x1, y1, x2, y2 : single; color : LongWord);
var
  I: Integer;
begin
setColor(color);
...
procedure TSMEPrmitives.Line(x1, y1, x2, y2 : single; r,g,b:single);
var
  I: Integer;
begin
fcolor.r:=r;
...
procedure TSMEPrmitives.HLine(x1, x2, y : single; color : LongWord);

begin


end;
procedure TSMEPrmitives.FillCircle(x, y, r : single; color : LongWord);
begin

end;


procedure TSMEPrmitives.FillEllipse(x, y, rx, ry : single; color : LongWord);
begin
...
procedure TSMEPrmitives.CubicBezierLine(x1, y1, x2, y2, c1x, c1y, c2x, c2y:single; segments : integer; color : LongWord);
var
  i:integer;
   x, y : single;
  delta, time : single;
  ...
  function GetBezier(p1, p2, c1, c2, time : single) : single;
  begin
    t1_1:= 1- time;
    t1_2:= t1_1* t1_1;
    t1_3:= t1_1* t1_2;
    t2:= time* time;
    t3:= t2* time;
    GetBezier:= t1_3* p1+ 3* time* t1_2* c1+ 3* t2* t1_1* c2+ t3* p2;
  end;
...
begin
  ...
  delta:= 1/ segments;
  lastx:= GetBezier(x1, x2, c1x, c2x, time);
  lasty:= GetBezier(y1, y2, c1y, c2y, time);
  for i:= 0 to segments do begin
    x:= GetBezier(x1, x2, c1x, c2x, time);
    y:= GetBezier(y1, y2, c1y, c2y, time);
    time:= time+ delta;
...
procedure TSMEPrmitives.Rect(x1, y1, x2, y2 : single; color : LongWord);
var tmp : single;
begin
setColor(color);
...
procedure TSMEPrmitives.VLine(y1, y2, x : single; color : LongWord);
var
  y, tmp : single;
begin
  if y1> y2 then begin
...
procedure TSMEPrmitives.setColor(color:Uint32) ;
var
    cr,cg,cb,ca:Single;
begin
      COLOR_UnRGBAF(color,cr,cg,cb,ca);
...
procedure TSMEPrmitives.Bind(mat:Matrix);
begin
  shader.bind;
...
procedure TSMEPrmitives.Circle(x, y:single;r :single; color : LongWord);
var
    Quality,i:integer;
    k : Single;
begin
    ...
    k := 360 / Quality;
     setcolor(color);
      for i := 0 to Quality - 1 do
        begin
          glVertex2f( X + R * cos( i * k ) , Y + R * sin( i * k )  );
          glVertex2f( X + R * cos( ( i + 1 ) * k ) , Y + R * sin( ( i + 1 ) * k )  );
        end


end;

procedure TSMEPrmitives.Ellipse(x, y, rx, ry : single; color : LongWord);
begin

end;

procedure TSMEPrmitives.glColor3f(r,g,b: GLfloat);
begin
Fcolor.r:=r;
Fcolor.g:=g;
Fcolor.b:=b;
...
procedure TSMEPrmitives.glVertex2f(x, y: GLfloat);
  var
    vertex : pColorvertex;
begin
...
procedure TSMEPrmitives.Ubind;
begin
if bSize = 0 Then exit;
SetBlendMode(SDL_BLENDMODE_NONE);
glEnableVertexAttribArray(ATTRIBUTE_POSITION);
glEnableVertexAttribArray(ATTRIBUTE_COLOR);
glVertexAttribPointer(ATTRIBUTE_POSITION, 2, GL_FLOAT, GL_FALSE,  sizeof (Colorvertex), @bVertices[0].x);
glVertexAttribPointer(ATTRIBUTE_COLOR, 4, GL_FLOAT, GL_FALSE,  sizeof (Colorvertex), @bVertices[0].color);
glDrawArrays( GL_LINES, 0, bSize );
...
constructor TSMEPrmitives.Create;
var
i,n,x:integer;
begin
setcolor($FFFFFFFF);
shader:=TSMEColorShader.Create;
shader.setString(color_pixel_shader,color_frag_shader);
end;
destructor TSMEPrmitives.Destroy;
begin
shader.Destroy;
end;
//**************************************
...
glVertexAttribPointer(ATTRIBUTE_POSITION, 2, GL_FLOAT, GL_FALSE, SizeOf(Colorvertex), @vertices[0].x);
//glEnableVertexAttribArray(ATTRIBUTE_POSITION);
glVertexAttribPointer(ATTRIBUTE_COLOR, 4, GL_FLOAT, GL_FALSE,  SizeOf(Colorvertex), @vertices[0].color);
//glEnableVertexAttribArray(ATTRIBUTE_COLOR);
...
procedure TSMECanvas.Triangles(Points: array of Colorvertex; NumPoints: Integer);
begin
glVertexAttribPointer(ATTRIBUTE_POSITION, 2, GL_FLOAT, GL_FALSE, SizeOf(Colorvertex), @Points[0].x);
glVertexAttribPointer(ATTRIBUTE_COLOR, 4, GL_FLOAT, GL_FALSE,  SizeOf(Colorvertex), @Points[0].color);
glDrawArrays(GL_TRIANGLE_FAN, 0, NumPoints)
end;
procedure TSMECanvas.Lines(Points: array of Colorvertex; NumPoints: Integer);
begin
glVertexAttribPointer(ATTRIBUTE_POSITION, 2, GL_FLOAT, GL_FALSE, SizeOf(Colorvertex), @Points[0].x);
glVertexAttribPointer(ATTRIBUTE_COLOR, 4, GL_FLOAT, GL_FALSE,  SizeOf(Colorvertex), @Points[0].color);
