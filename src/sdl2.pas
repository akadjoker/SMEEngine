unit SDL2;
{
  ...
  Pascal-Header-Conversion MOdifications
  Copyright (C) 2013 Luis Santos  aka Djoker
...
const
 ...
 {$IFDEF MSWINDOWS}
  SDL_LibName = 'SDLSimple.dll';
  {$ELSE}
  SDL_LibName = 'libSDL_Simple.so';
  {$ENDIF}
...
function SDL_RWFromFile(const _file: pChar; const mode: pChar): PSDL_RWops; cdecl; external {$IFDEF GPC} name 'SDL_RWFromFile' {$ELSE} SDL_LibName {$ENDIF};
...
procedure  SDL_Log(const char :pchar);  cdecl; external {$IFDEF GPC} name 'SDL_Log' {$ELSE} SDL_LibName {$ENDIF};
procedure  SDL_LogVerbose(category:integer;const char :pchar);  cdecl; external {$IFDEF GPC} name 'SDL_LogVerbose' {$ELSE} SDL_LibName {$ENDIF};
procedure  SDL_LogDebug(category:integer;const char :pchar);  cdecl; external {$IFDEF GPC} name 'SDL_LogDebug' {$ELSE} SDL_LibName {$ENDIF};
procedure  SDL_LogInfo(category:integer;const char :pchar);  cdecl; external {$IFDEF GPC} name 'SDL_LogInfo' {$ELSE} SDL_LibName {$ENDIF};
procedure  SDL_LogWarn(category:integer;const char :pchar);  cdecl; external {$IFDEF GPC} name 'SDL_LogWarn' {$ELSE} SDL_LibName {$ENDIF};
procedure  SDL_LogError(category:integer;const char :pchar);  cdecl; external {$IFDEF GPC} name 'SDL_LogError' {$ELSE} SDL_LibName {$ENDIF};
procedure  SDL_LogCritical(category:integer;const char :pchar);  cdecl; external {$IFDEF GPC} name 'SDL_LogCritical' {$ELSE} SDL_LibName {$ENDIF};


PROCEDURE SDL_LogMessage(category:integer;priority:SDL_LogPriority;const fmt:pchar);  cdecl; external {$IFDEF GPC} name 'SDL_LogMessage' {$ELSE} SDL_LibName {$ENDIF};
...
SDL_MessageBoxColorScheme=record
    ...
    colors:array[0..ord(SDL_MESSAGEBOX_COLOR_MAX)]of SDL_MessageBoxColor;
end;
...
SDL_MessageBoxData=record
    ...
    smessage:pchar;                //**< UTF-8 message text */
...
function SDL_ShowSimpleMessageBox(flags:SDL_MessageBoxFlags;const title:pchar;const smessage:pchar;window:PSDL_Window):integer;  cdecl; external {$IFDEF GPC} name 'SDL_ShowSimpleMessageBox' {$ELSE} SDL_LibName {$ENDIF};
...
function SDL_ShowMessageBox(const messageboxdata:PSDL_MessageBoxData;var buttonid:integer):integer;  cdecl; external {$IFDEF GPC} name 'SDL_ShowMessageBox' {$ELSE} SDL_LibName {$ENDIF};
...
function SDL_LoadObject(const sofile:pchar):pointer; cdecl; external  SDL_LibName;
...
function SDL_LoadFunction(handle:pointer;   const name:pchar):pointer; cdecl; external  SDL_LibName ;
