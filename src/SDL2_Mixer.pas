...
{*******************************************************************************
  ...
  SDL2_Mixer.pas  v1.0  29/07/2013
...
{$IFDEF  MSWINDOWS		}
  SDL_MixerLibName = 'SDLSimple.dll';
  {$ELSE}
  SDL_MixerLibName = 'libSDL_Simple.so';
  {$ENDIF}
...
function  Mix_GetNumChunkDecoders:integer;cdecl; external SDL_MixerLibName;
function  Mix_GetChunkDecoder(index:integer):PAnsiChar;cdecl; external SDL_MixerLibName;
function  Mix_GetNumMusicDecoders:integer;cdecl; external SDL_MixerLibName;
function  Mix_GetMusicDecoder( index:integer):PAnsiChar;cdecl; external SDL_MixerLibName;
...
var
   fichero : PSDL_RWops;
begin
  fichero := SDL_RWFromFile( filename, 'rb' );
  if fichero <> nil then
     result := Mix_LoadWAV_RW( fichero, 1 );
end;
